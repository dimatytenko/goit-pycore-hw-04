...
def get_line_symbol(is_last_item):
    return '┗' if is_last_item else '┣'

def print_structure(current_path, indent=""):
            items = sorted(current_path.iterdir(), key=lambda x: x.is_file())
            
            for i, item in enumerate(items):
                is_last_item = (i == len(items) - 1)
                line_symbol = get_line_symbol(is_last_item)

                if item.is_dir():
                    print(f"{indent}{line_symbol} 📦 {Fore.BLUE}{item.name}/{Style.RESET_ALL}")
                    print_structure(item, indent + ("" if is_last_item else "┃ "))
                else:
                    print(f"{indent}{line_symbol} 📜 {Fore.GREEN}{item.name}{Style.RESET_ALL}")
